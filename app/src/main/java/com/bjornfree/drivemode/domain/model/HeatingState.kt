...
data class HeatingState(
    ...
    /**
     * Флаг что настройки только что изменены программно.
     * true = настройки изменены через UI (НЕ считать следующее отличие вмешательством)
     * false = нормальный режим работы
     *
     * Используется чтобы различать:
     * - Изменение настроек приложения (температурный порог, адаптивный режим и т.д.) - НЕ вмешательство
     * - Прямое изменение уровня HVAC через штатную систему авто - вмешательство
     */
    val settingsJustChanged: Boolean = false
) {
    /**
